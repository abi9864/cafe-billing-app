...
router.post('/:id/adjust', authenticate, authorize('admin', 'manager'), asyncHandler(async (req, res) => {
  ...
  const before = await query('SELECT quantity_on_hand, reorder_level FROM inventory_items WHERE id = $1', [req.params.id]);
  if (before.rows.length === 0) throw notFound('Inventory item not found');
  const wasAboveReorderLevel = before.rows[0].reorder_level == null
    || parseFloat(before.rows[0].quantity_on_hand) > parseFloat(before.rows[0].reorder_level);
  ...
  // Only fire an alert on the crossing (was fine, now low) — not every
  // subsequent adjustment while it's already sitting below reorder level.
  const nowAtOrBelow = item.reorder_level != null && parseFloat(item.quantity_on_hand) <= parseFloat(item.reorder_level);
  console.log(`Stock adjust "${item.item_name}": wasAboveReorderLevel=${wasAboveReorderLevel}, nowAtOrBelow=${nowAtOrBelow}, qty=${item.quantity_on_hand}, reorder_level=${item.reorder_level}`);
  if (wasAboveReorderLevel && nowAtOrBelow) {
    sendLowStockAlert(item); // fire-and-forget, doesn't block the response
  }
