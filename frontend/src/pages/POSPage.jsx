...
function MenuItemCard({ item, onAdd }) {
  ...
  return (
    <>
      ...
      {/* Variant picker modal — rendered as a SIBLING of the card, not a child.
          The card above has active:scale-95 (a transform on :active). Per the
          CSS spec, :active applies to an element's ancestors too while a
          descendant is being pressed — so if this modal were nested inside
          the card div, pressing a variant button would transform that
          ancestor at the exact moment of the click. A `position: fixed`
          descendant of a transformed ancestor loses true viewport-relative
          positioning, which silently shifts the real click target a few
          pixels away from where it's rendered — the button "blinks" but the
          click lands just outside it. Keeping the modal as a sibling avoids
          the card's transform entirely. */}
      {showVariants && (
